...
def analyze_document(extracted_text: str, client_name: str, tax_year: int, required_docs: list) -> dict:
    required_types = [d["type"] for d in required_docs]
    required_list_str = ", ".join(f'"{t}"' for t in required_types)

    prompt = f"""You are a tax document analyzer for a CPA firm.

Client on file: {client_name}
Required tax year: {tax_year}
...
- detected_year: the tax year found in the document as an integer, or null if not found
- detected_client: the taxpayer name found in the document as a string, or null if not found
- type_correct: boolean — does identified_type match any document in the required types list?
- year_correct: boolean — does detected_year equal {tax_year}?
- client_correct: boolean — does detected_client closely match "{client_name}"?
